...
class AssignClassTeacherController extends Controller
{
    ...
    public function insert(Request $request)
    {
        ...
        {
            return redirect()->back()->with('error', 'Due to some error please try again letter');
        }
    }
...
public function update_single($id, Request $request)
    {
        $getAlreadyFirst = AssignClassTeacherModel::getAlreadyFirst($request->class_id, $request->teacher_id);
                    if(!empty($getAlreadyFirst))
                    {
                        $getAlreadyFirst->status = $request->status;
                        $getAlreadyFirst->save(); 
                        return redirect('admin/assign_class_teacher/list')->with('success', "Assign Class to teacher Successfully Updatad");
                    }
                    else
                    {
 ...
 public function delete($id)
    {
        $save = AssignClassTeacherModel::getSingle($id);
        $save->delete(); 
        return redirect()->back()->with('success', " Assign to Class Teacher Successfully Deleted");
    }
